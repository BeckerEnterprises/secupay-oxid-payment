    ...
    class secupay_api_response {
        ...
        function check_response($log_error = false) {
            ...
            if (strtolower($this->status) != 'ok') {
                if ($log_error)
                    secupay_log::log("secupay_api_response status: ", $this->status);
                return false;
            };

            if (count($this->errors) > 0) {
                if ($log_error)
                    secupay_log::log("secupay_api_response error: ", $this->errors);
                return false;
            }

            if (count($this->data) == 0) {
                if ($log_error)
                    secupay_log::log("secupay_api_response error: no data in response");
                return false;
            }
        ...
        function get_status($log_error = false) {
            if ($log_error)
                secupay_log::log("secupay_api_response get_status: " . $this->status);
        ...
        function get_error_message($log_error = false) {
            ...
            if (isset($this->errors)) {
                ...
                if ($log_error)
                    secupay_log::log("secupay_api_response get_error_message: " . $message);
                return $message;
            } else {
                return false;
            }
        ...
        function get_error_message_user($log_error = false) {
            ...
            if (isset($this->errors)) {
                ...
                if ($log_error)
                    secupay_log::log("secupay_api_response get_error_message_user: " . $message);
                return $message;
            } else {
                return false;
            }
    ...
    class secupay_table {
        ...
        static function getIframeUrlId($hash, $iframe_url, $log_error = false) {
            ...
            if (isset($iframe_url_id) && is_numeric($iframe_url_id)) {
                ...
                secupay_log::log($log_error, 'secupay_api, getIframeUrlId queries', $sSQL_select, $sSQL_insert, $sSQL_last_inserted_id);
                return 0;
            }
        }
