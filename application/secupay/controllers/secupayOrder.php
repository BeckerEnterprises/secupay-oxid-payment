...
class secupayOrder extends secupayOrder_parent
{
    ...
    public function execute()
    {
        ...
        if (isset($payment_type) && $payment_type) {
            ...
            if ($myConfig->getRequestParameter('ord_custinfo') !== null
                && !$myConfig->getRequestParameter(
                    'ord_custinfo'
                )
                && $this->isConfirmCustInfoActive()) {
    ...
    protected function createSecupayTransaction($oOrder)
    {
        ...
        $this->getSession()
             ->deleteVariable('secupay_iframe_url');
        $this->getSession()
             ->deleteVariable('secupay_hash');
        $this->getSession()
        ...
        if (empty($oID)) {
            $sGetChallenge = $this->getSession()
                                  ->getVariable('sess_challenge');
            $oID           = $sGetChallenge;
            secupay_log::log($this->_secupay_log, "secupayOrder, get oID from Session: ", $oID);
        }

        $active_experience = $this->getConfig()
                                  ->getConfigParam('secupay_experience');
        secupay_log::log($this->_secupay_log, 'oID: ', $oID);

        $pmnt_id = $oBasket->getPaymentId(); // definiert Zahlart

        $modus = $this->getConfig()
                      ->getConfigParam('secupay_blMode');

        if (!oxRegistry::getConfig()
                       ->isSSL()) {
            $uri_prefix = oxRegistry::getConfig()
                                    ->getConfigParam('sShopURL');
        } else {
            $uri_prefix = oxRegistry::getConfig()
                                    ->getConfigParam('sSSLShopURL');
        }
        ...
        $daten['url_success'] = $this->getConfig()
                                     ->getSslShopUrl()
            . 'index.php?cl=order&fnc=processSecupaySuccess&sDeliveryAddressMD5=' . order::getDeliveryAddressMD5();
        $daten['url_failure'] = $this->getConfig()
                                     ->getSslShopUrl() . 'index.php?cl=order&fnc=processSecupayFailure';
        $daten['url_push']    = $this->getConfig()
                                     ->getSslShopUrl() . 'index.php?cl=secupay_push';

        $daten['shop']         = "oxid ce";
        $daten['shopversion']  = $this->getConfig()
        ...
        } elseif (isset($lang_abbr) && $lang_abbr === 'de') {
        ...
        $daten['country']                = $oDB->getOne(
            "SELECT oxisoalpha2 FROM oxcountry WHERE oxid = ?",
            [$sCountryId]
        );
        ...
        foreach ($basketcontents as $item) {
            ...
            $product->article_number = $item->getArticle()
                                            ->getFieldData('oxartnum');
            $product->name           = $item->getTitle();
            $product->model          = $item->getArticle()
                                            ->getFieldData('oxshortdesc');
            $product->ean            = $item->getArticle()
                                            ->getFieldData('oxean');
            $product->quantity       = $item->getAmount();
            $product->price          = $item->getArticle()
                                            ->getPrice(1)
                                            ->getBruttoPrice() * 100;
            $product->total          = $item->getPrice()
                                            ->getBruttoPrice() * 100;
            $product->tax            = $item->getPrice()
                                            ->getVat();
            $products[]              = $product;
        }
        ...
        if ($oDelAd) {
            ...
            $delivery_address->country     = $oDB->getOne(
                "SELECT oxisoalpha2 FROM oxcountry WHERE oxid = ?",
                [$sDelCountry]
            );
        ...
        if ($api_return && $api_return->check_response()) {
            ...
            secupay_table::createTransactionEntry(
                $_req_data,
                $_ret_data,
                $_hash,
                $ox_payment_id,
                $oID,
                $amount,
                $api_return->get_iframe_url(),
                $this->_secupay_log
            );
            secupay_table::createStatusEntry($_hash, '', 'erstellt', $this->_secupay_log);

            $this->getSession()
                 ->setVariable('secupay_iframe_url', $api_return->get_iframe_url());
            $this->getSession()
                 ->setVariable('secupay_hash', $_hash);
            ...
            oxRegistry::getUtils()
                      ->redirect(
                          oxRegistry::getConfig()
                                    ->getShopSecureHomeUrl() . 'cl=order&fnc=processSecupayIframe',
                          true,
                          302
                      );
            ...
            $this->getSession()
                 ->setVariable('sp_err_msg', $error_message);
    ...
    protected function get_purpose()
    {
        ...
        $_custom_name = $this->getConfig()
                             ->getConfigParam('secupay_shopname_ls');
    ...
    protected function _getNextStep($iSuccess)
    {
        ...
        if (isset($payment_type) && $payment_type) {
            ...
            if ($iSuccess === 'secupayOK') {
                ...
                try {
                    $hash = $this->getSession()
                                 ->getVariable('secupay_hash');
                    ...
                    secupay_log::log(
                        $this->_secupay_log,
                        "secupayOrder, _getNextStep secupayOK - Exception:",
                        $oEx->getMessage()
                    );
                    secupay_log::log(
                        $this->_secupay_log,
                        "secupayOrder, _getNextStep secupayOK - Trace:",
    ...
    protected function cancelSecupayTransaction($hash, $comment = null)
    {
        if (isset($hash) && !empty($hash)) {
            ...
            secupay_log::log(
                $this->_secupay_log,
                "secupayOrder, cancelSecupayTransaction - api return data: ",
                $_ret_data
            );
    ...
    public function processSecupaySuccess()
    {
        ...
        if (isset($hash) && $this->validateSecupayTransaction($hash)) {
            $iSuccess = 'secupayOK';
            $this->getSession()
                 ->setVariable('secupay_success', true);
        } else {
            $this->getSession()
                 ->setVariable('secupay_success', false);
    ...
    protected function validateSecupayTransaction($hash)
    {
        ...
        if (isset($hash)) {
            ...
            if ($api_return && $api_return->check_response() && isset($api_return->data->status)
                && ($api_return->data->status == 'accepted' || $api_return->data->status == 'authorized'
                    || ($payment_type == 'prepay' && $api_return->data->status == 'scored'))
                && isset($amount_received)
                && isset($amount_basket)
                && intval(strval($amount_received)) === intval(strval($amount_basket))) {
                ...
                secupay_log::log(
                    $this->_secupay_log,
                    "amount_basket: " . $amount_basket . " amount_received " . $amount_received
                );

                if (isset($amount_received) && isset($amount_basket)
                    && intval(strval($amount_received)) === intval(
                        strval($amount_basket)
                    )) {
    ...
    public function getIframeUrl()
    {
        ...
        $iframe_url = $this->getSession()
                           ->getVariable('secupay_iframe_url');
        if (empty($iframe_url)) {
            oxRegistry::getUtils()
                      ->redirect(
                          oxRegistry::getConfig()
                                    ->getShopHomeURL() . 'cl=order&fnc=processSecupayFailure',
                          true,
                          302
                      );
            return;
    ...
    public function should_secupay_warn_delivery()
    {
        ...
        if (isset($payment)) {
            ...
            if ($delivery_adress_option_name
                && $this->getConfig()
                        ->getConfigParam($delivery_adress_option_name) == 2) {
                $oBasket = $this->getSession()
                                ->getBasket();
                $oID     = $oBasket->getOrderId();
                $oOrder  = oxnew("oxOrder");
