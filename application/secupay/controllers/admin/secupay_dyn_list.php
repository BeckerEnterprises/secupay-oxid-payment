...
class secupay_dyn_list extends oxAdminList
{
    ...
    public function render() {
        ...
        while($row = $rows_list->FetchRow()){
            ...
            if(isset($row['hash'])  && strlen($row['hash']) > 5 && $row['payment_method']=="secupay_invoice") {
                $capture_action = '<a target="_blank" title="secupay.Rechnungskauf als f&auml;llig markieren" href="' . SECUPAY_URL . $row['hash'] . '/capture/' . $apikey . '" class="sp_capture"><span class="ico"></span></a>';
            } else {
                $capture_action = "";
            }
            ...
            if(isset($row['hash']) && strlen($row['hash']) > 5) {
                $row['actions'] .= '<a title="Status abfragen" onclick="SP.status_request(\''.$row['hash'].'\');" class="zoomText"><span class="ico"></span></a>';
            }
    ...
    public function getPath() {
        return $this->getConfig()->getSslShopUrl().'modules/secupay/controllers/admin/';
    }
    ...
    public function fetch_secupay_status($hash, $apikey){
        ...
        $secupay_log = $this->getConfig()->getConfigParam('secupay_blDebug_log');
