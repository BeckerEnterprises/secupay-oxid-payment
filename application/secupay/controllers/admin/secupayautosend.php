...
class secupayautosend extends secupayautosend_parent
{
    ...
    public function autosend()
    {
        $this->_secupay_log = $this->getConfig()
                                   ->getConfigParam('secupay_blDebug_log');
        $active_autosend    = $this->getConfig()
                                   ->getConfigParam('secupay_invoice_autosend');
        $active_autoinvoice = $this->getConfig()
                                   ->getConfigParam('secupay_invoice_autoinvoice');
        $active_tautosend   = $this->getConfig()
                                   ->getConfigParam('secupay_tracking_autosend');
        ...
        if ($active_autosend or $active_tautosend) {
            ...
            foreach ($autosends as $autosend) {
                ...
                if ($api_return->status == 'ok' or utf8_decode($api_return->errors[0]->message)
    ...
    public function getCarrier($trackingnumber, $provider)
    {
        if (preg_match(
            ...
            || preg_match(
                "/NN\s?\d{2}\s?\d{3}\s?\d{3}\s?\d(?=DE(\s)?\d{3})/",
                $trackingnumber
            )
