<?php
/**
 *    This file was edited slightly by Secupay
 *
...
//        $oPdf = oxNew( 'oxPDF' );
    ...
    private function addSecupayInfo(&$oPdf, &$iStartPos)
    {
        ...
        $days_until_billdate = intval($this->getConfig()->getConfigParam('SECUPAY_DAYS_TO_BILLDATE'));
        if(empty($days_until_billdate) || !is_numeric($days_until_billdate)) {
            $days_until_billdate = 10;
        }
        //secupay_log::log(true, print_r($invoice_data, true));
        if(!empty($invoice_data)) {
            ...
            if($transactionInfo['payment_method']=='secupay_invoice'){
                $oPdf->text(15, $iStartPos - 4, $this->translate( 'ORDER_OVERVIEW_PDF_PAYUPTO' ). date('d.m.Y', strtotime('+' . $days_until_billdate . ' day', strtotime($billdate))));
            }
            ...
            $oPdf->text(15, $iStartPos + 8, $this->_translateSecupayString( str_replace( '[recipient_legal]', $invoice_data->recipient_legal, $this->_translateSecupayString('SECUPAY_ADDRESS_INVOICE')) , true) );
            $oPdf->setFont( $oPdfBlock->getFont(), 'B', 10 );
            $oPdf->text(15, $iStartPos + 12, $this->_translateSecupayString('SECUPAY_PAYMENT_HEADER'));
            $oPdf->setFont( $oPdfBlock->getFont(), '', 10 );
            $oPdf->text(15, $iStartPos + 22, $this->_translateSecupayString('SECUPAY_RECIPIENT') . $this->_translateSecupayString( $invoice_data->transfer_payment_data->accountowner , true) );
            $oPdf->text(15, $iStartPos + 26, $this->_translateSecupayString('SECUPAY_IBAN') . $invoice_data->transfer_payment_data->iban . ', ' .
                                              $this->_translateSecupayString('SECUPAY_BIC') . $invoice_data->transfer_payment_data->bic);
            $oPdf->text(15, $iStartPos + 30, $this->_translateSecupayString('SECUPAY_BANK_NAME') . $this->_translateSecupayString( $invoice_data->transfer_payment_data->bankname, true));                                              
            ...
            $oPdf->setFont( $oPdfBlock->getFont(), 'B', 10 );
            $oPdf->text(15 + $width, $iStartPos + 34, ' ' . $this->_translateSecupayString( $invoice_data->transfer_payment_data->purpose, true ));
            // set font back to normal
           if($transactionInfo['payment_method']=='secupay_invoice'){
               $oPdf->setFont($oPdfBlock->getFont(), '', 10 );
               $oPdf->text(15, $iStartPos + 44, $this->_translateSecupayString('SECUPAY_INFO_BOTTOM_1'));
               $oPdf->text(15, $iStartPos + 48, $this->_translateSecupayString('SECUPAY_INFO_BOTTOM_2') . ' ' . $transactionInfo['iframe_url']);
               ...
               if (!empty($invoice_data->payment_link)) {
                   ...
                   $iMargin  = 195 - $aSize[0] * 0.2;
                   $oPdf->image($qr_code_img, $iMargin, $iStartPos + 14, $aSize[0] * 0.2, $aSize[1] * 0.2, substr(strrchr($aSize['mime'], '/'), 1), $invoice_data->payment_link);
               }
           }
            $iStartPos += 48;
    ...
    private function _translateSecupayString($translation_string, $skip_translation = false) {
        $use_utf8 = $this->getConfig()->getConfigParam('SECUPAY_USE_UTF8');
